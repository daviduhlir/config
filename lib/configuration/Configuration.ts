...
export class Configuration<T extends JsonValidatorObjectChildsSchema> {
   ...
   * @returns
   */
  public static async load<T extends JsonValidatorObjectChildsSchema>(schema: T, filepaths: string[], override: Partial<T> = {}) {
  ...
  protected validate() {
    try {
      this.internalData = JsonValidator.validate(this.internalData, {
